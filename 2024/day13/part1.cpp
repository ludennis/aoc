...
bool has_no_remainder(int a, int b)
{
    a = abs(a);
    b = abs(b);
    while (a >= b)
...
int win_prize_with_minimum_token(const Pair& a, const Pair& b, const Pair& prize)
{
    if (!is_prize_reachable(a, b, prize))
        return 0;

    int an = (prize.first * b.second - b.first * prize.second) /
            (b.second * a.first - a.second * b.first);
    int bn = (prize.first - a.first * an) / b.first;
